...
def predictor(input_library, iteration, plot=True):
    ...
    print(f"here is the prediction value {prediction} and its type {type(prediction)}")
...
x_mut = np.concatenate([x_mut_l, np.broadcast_to(no_padding_seq_motif, (lib_size, seq_length_m, 4)), x_mut_r], axis=1)
...
# save the figure
plt.savefig("/home/nagle/final_version/output/2tail_mut_preds.png")
