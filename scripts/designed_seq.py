...
from plotting import (
    ...
    plot_kde_grid_surrogate_vs_gt, plot_coef_dists_per_gt,
)
...
def make_4_gt_eval_libraries(
    *, x_base, input_seq_str, gt_params, nonlin_name, nonlin_kwargs, out_dir,
    ...
    Samples `n_pool` random sequences (scored cheaply via the analytic GT
    model), then downsamples each GT key to exactly `target_n` sequences with
    a uniform score distribution.
    ...
    sees consistently scaled inputs across all 2M sequences.
    ...
    # ── Generate n_pool random sequences as uint8 indices (cheap: n_pool×L bytes)
    ...
    # Pool-level standard deviations (used consistently across all chunks)
    ...
    # ── Pass 2: apply global normalisation + nonlinearity in chunks
    ...
    for start in range(0, n_pool, chunk_size):
        ...
        pool_scores["nonlin_additive_pairwise"][start:end] = apply_global_nonlin(
            sap / std_addpair, nonlin_name, nonlin_kwargs,
        ).reshape(-1)
...
def random_mut_library_round0(rho_storage, coef_store, run_idx, epmapmin, epmapmax):
    ...
    nonlin_family  = "mix_tanh"
    nonlin_kwargs  = gt_bundle["nonlin_kwargs"]["mix_tanh"]
    ...
    nonlin_family_for_plots = "mix_tanh"
