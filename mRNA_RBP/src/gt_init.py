...
_MRNA_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
_REPO_ROOT = os.path.dirname(_MRNA_ROOT)
sys.path.insert(0, _REPO_ROOT)
sys.path.insert(0, os.path.join(_REPO_ROOT, "external", "residualbind"))
...
# ---------------------------------------------------------------------------
# Negative-control ground truth: motif-additive, no pairwise interactions
# ---------------------------------------------------------------------------

class MrnaNegativeControlGroundTruth:
    """
    HuR-like motif-only control: strong additive effects at a privileged motif,
    near-neutral background positions, and no pairwise interactions.
    It uses the same WT-anchored sigmoid as the structured Synthetic GT.

    Nonlinearity: same sigmoid form and same hand-picked constants as
    MrnaRbpGroundTruth (a=1, b=1, c=1.5, d=0), WT-anchored so WT=0.

    Interface mirrors MrnaRbpGroundTruth: alpha, beta, edges, wt_one_hot(),
    wt_activity(), score_all(x), __call__(x). All scores ≤ 0, WT = 0.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self._J = np.zeros((L, L, 4, 4), dtype=np.float32)

        # Interface parity with MrnaRbpGroundTruth: generate_pairwise_lib,
        # evaluate.py, train_surrogate.py, and viz.py all read gt.stem_pairs /
        # gt.motif_positions directly (not the sequence_config_for_oracle
        # values, which are [] for this oracle since topology is per-instance
        # random and unknowable ahead of construction).
        self.stem_pairs: list = []
        self.motif_positions = list(motif_positions)
    ...
    @property
    def beta(self) -> dict:
        """Dict {(i, j): (4, 4) array} for each sparse randomly-selected edge."""
        return {(int(i), int(j)): self._J[int(i), int(j)] for (i, j) in self._edges.tolist()}

    @property
    def edges(self) -> np.ndarray:
        """(E, 2) int32 array of sparse randomly-selected edge indices."""
        return self._edges
    ...
    def __repr__(self) -> str:
        return (
            f"MrnaNegativeControlGroundTruth(L={len(self.seq)}, "
            f"motif={len(self.motif_positions)}, pairwise_edges=0, "
            f"sigmoid=(a={self._a}, b={self._b}, c={self._c}, d={self._d}), "
            f"wt_activity={self.wt_activity():.3f})"
        )
