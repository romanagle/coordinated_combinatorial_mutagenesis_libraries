...
The older notebook `mRNA_RBP/notebooks/reproduce_gt_pipeline.ipynb` is
display/cache-first context only. It should not be treated as the authoritative
pipeline until it is rewritten to call the script directly.
...
```text
mRNA_RBP/lib_size_spearman.py --out_json mRNA_RBP/outputs/lib_size_spearman_results_type3.json --recompute_saturated --saturated_only --gt_keys additive additive_pairwise nonlin_additive nonlin_additive_pairwise
mRNA_RBP/plots/plot_library_distributions.py
mRNA_RBP/plots/plot_synthetic_rand_region_distributions.py
mRNA_RBP/plots/plot_scatter_by_mutcount.py
mRNA_RBP/plots/plot_rho_vs_libsize_type3.py
mRNA_RBP/plots/bar_surrogate_models_type3.py
mRNA_RBP/plots/plot_cross_mutrate.py --out_prefix synthetic_gt_
```
...
2. Synthetic random library plots are generated by
   `mRNA_RBP/plots/plot_synthetic_rand_region_distributions.py`. They reuse the
...
4. Scatter-by-mutation-count reads a saved prediction artifact. For
   ResidualBind VTS1, `mRNA_RBP/generate_residualbind_vts1_scatter_predictions.py`
