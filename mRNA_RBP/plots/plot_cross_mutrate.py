...
Two figures:
  ...
  (b) cross_mutrate_heatmap.png -- small-multiples 3x3 train x test heatmap
      of mean Spearman |rho| at the largest lib size, one panel per config.
...
heatmap with no extra logic).
    ...
    python mRNA_RBP/plot_cross_mutrate.py \
        --results_json "mRNA_RBP/outputs/ground_truth_collections/ResidualBind oracle MSI1/pipeline_workspace/outputs_residualbind_ensemble/cross_mutrate_results.json" \
...
from mRNA_RBP.evaluate import make_ssm_deltas_from_scores

_HERE = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
sys.path.insert(0, os.path.join(_HERE, ".."))

from mRNA_RBP.generate_libraries import MUT_RATES_PCT, LIB_SIZES
from mRNA_RBP.plots.provenance import stamp_figure
...
CFG_NAMES = [
    "additive", "additive + pairwise",
    "nonlinear additive", "nonlinear additive + pairwise",
]
