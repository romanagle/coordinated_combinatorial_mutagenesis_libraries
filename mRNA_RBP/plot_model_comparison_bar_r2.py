...
def _r2_surrogate(k, pct, n_lib, cfg_name, gt_key):
    """Pearson R² for surrogate on type2 and pairwise eval libs, instance k."""
...
def _r2_ssm(k, gt_key):
    """Pearson R² for SSM predictor on type2 and pairwise eval libs, instance k."""
    inst_dir = os.path.join(INST_DIR, f"instance_{k:02d}")
    t2_path  = os.path.join(inst_dir, "type2.npz")
    pw_path  = os.path.join(inst_dir, "pairwise_lib.npz")
...
def main():
    ...
    for m in models:
        ...
        print(f"  {m:35s}  type2 R²={tm:.3f}  pairwise R²={pm:.3f}")
    ...
    legend_handles = [
        Patch(facecolor=COLOR_T2, alpha=0.85, label="Type 2 eval library"),
