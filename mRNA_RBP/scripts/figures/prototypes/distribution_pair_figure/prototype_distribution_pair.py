"""PROTOTYPE / THROWAWAY: paired distributions from real-oracle caches."""
...
OUT = Path(__file__).resolve().parents[5] / "mRNA_RBP" / "prototypes" / "distribution_pair_figure"
CACHES = {
    "VTS1": OUT / "vts1_replacement_random_library_scores.npz",
    "HuR": ROOT / "mRNA_RBP/outputs/ground_truth_collections/ResidualBind oracle HuR/libraries_used_for_figures/random_region_score_cache/hur_natural_random_library_scores.npz",
}
    ...
    for name, path in CACHES.items():
        ...
        for pct, label in RATES:
            ...
            rates[label] = {
                ...
                "motif": score[motif_hit & ~stem_hit],
            }
        data[name] = rates
    return data
...
def variant_a(data):
    ...
    lo = min(np.percentile(vals, 0.2) for vals in plotted if len(vals))
    hi = max(np.percentile(vals, 99.8) for vals in plotted if len(vals))
    ...
    for ax, name in zip(axes, ("VTS1", "HuR")):
        ...
        title = "VTS1" if name == "VTS1" else name
    ...
    fig.text(.5, .015, "Dotted: WT = 0  •  Mutually exclusive motif-only and stem-only classes  •  Both/neither omitted", ha="center", fontsize=9)
