...
Models (x-axis groups):
  SSM | Additive | Additive+Pairwise | NL Additive+Pairwise

Eval libraries (3 bars per group, centered):
...
from mRNA_RBP.src.evaluate import make_ssm_deltas, predict_ssm
...
EVAL_KEYS = ["random", "type2", "pairwise"]
...
def _cache_data(score_cache):
    ...
    eval_ids = {name: d[f"nuc_ids_{name}"] for name in ("type2", "pairwise")}
...
def _save_combined_plot(values, library_sizes, out_stem, title_prefix="Synthetic GT",
    ...
    for key in EVAL_KEYS:
        ...
        _annotate_bars(ax, bars, "spearman")
    ...
    fig.legend(handles, legend_labels, loc="upper center", ncol=3,
               fontsize=9, framealpha=0.92, bbox_to_anchor=(0.5, 0.97))
