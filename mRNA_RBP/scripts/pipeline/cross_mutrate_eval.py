"""mRNA_RBP/scripts/pipeline/cross_mutrate_eval.py
...
For every (instance k, train_pct, n_lib) nonlinear additive + pairwise condition already
...
Usage:
    python mRNA_RBP/scripts/pipeline/cross_mutrate_eval.py \
...
_ORACLE_CHOICES = [
    MRNA_ORACLE, "mrna", "residualbind", "residualbind_ensemble",
...
# Cross-rate analyses use the manuscript's selected surrogate only. Kept as a
# list to preserve the existing result schema and plotting interface.
CFG_NAMES = [
...
def main():
    ...
    parser.add_argument("--oracle", default=MRNA_ORACLE, choices=_ORACLE_CHOICES)
