"""
mRNA_RBP/scripts/pipeline/generate_libraries.py
...
def generate_type3_lib(k: int, inst_dir: str, gt: MrnaRbpGroundTruth, seq: str,
                       gt_keys=None,
                       primary_key: str = "nonlin_additive_pairwise",
                       force: bool = False):
    gt_keys = gt_keys or GT_KEYS
    """Saturated additive + pairwise library: exhaustive singles and doubles.

    N = 3L single mutants + 9*C(L,2) double mutants.  ``rate_labels`` records
    the mutation count (only 1 or 2) of each row.  The candidate set is
    deterministic and identical across instances.  Saves inst_dir/type3.npz.
    """
    path = os.path.join(inst_dir, "type3.npz")
    if os.path.exists(path) and not force:
        print(f"  type3 exists — skip")
        return

    nuc_ids, mut_labels = generate_type3_nuc_ids(seq)
    X      = np.eye(4, dtype=np.float32)[nuc_ids]
    scores = gt.score_all(X)
    save_npz(path, nuc_ids, scores, gt.edges, gt_keys, extra={"rate_labels": mut_labels})
    print(f"  type3 saved  n={len(nuc_ids)}  "
          f"{primary_key} ∈ [{scores[primary_key].min():.4f}, "
          f"{scores[primary_key].max():.4f}]")
...
def run_pipeline(n_instances: int = N_INSTANCES, pool_size: int = N_LARGE,
    ...
    use_wt_activity = oracle_uses_wt_activity(oracle_name)
    ...
    for k in range(n_instances):
        ...
        if force_regen or not os.path.exists(gt_params_path):
            np.savez_compressed(
                ...
                wt_activity = np.array([wt_activity if use_wt_activity else "default"]),
                wt_seq  = np.array(seq),
            )
            print(f"[instance {k:02d}]  saved gt_params  edges={len(gt.edges)}")
        else:
        ...
        generate_type3_lib(k, inst_dir, gt, seq, gt_keys, primary_key, force=force_regen)
...
if __name__ == "__main__":
    ...
    _p.add_argument("--out_base", default=None,
                    help="Output directory. Defaults to outputs for synthetic GT or runs/<family>/<target>/<wt> for biological oracles")
    ...
    _p.add_argument("--wt_activity", choices=["high", "low"], default="high",
                    help="Natural-probe WT sequence context (VTS1/HuR/QKI) used for random-library distributions")
    _args = _p.parse_args()
