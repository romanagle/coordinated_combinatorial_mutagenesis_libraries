"""
mRNA_RBP/train_surrogate_varied_mutrate.py

Trains the nonlinear additive+pairwise MAVE-NN surrogate on a random
subsample of the ResidualBind-scored varied-mutation-rate library (mutation
counts 3-10; see generate_varied_mutrate_library.py and
score_residualbind_varied_mutrate.py), then saves the trained model to disk
so it can be reloaded as a standalone oracle (SurrogateMAVENNOracle in
oracles.py) without retraining. Supports either oracle's scored pool via
--oracle (residualbind_ensemble = MSI1, default; vts1_residualbind = real
VTS1/RNCMPT00111 -- see oracles.py).

Run with the squid conda environment:
    /home/nagle/miniconda3/envs/squid/bin/python3.7 mRNA_RBP/train_surrogate_varied_mutrate.py
    /home/nagle/miniconda3/envs/squid/bin/python3.7 mRNA_RBP/train_surrogate_varied_mutrate.py --oracle vts1_residualbind

Reads:  mRNA_RBP/outputs_<oracle>/instance_00/varied_mutrate/pool.npz
Writes: mRNA_RBP/outputs_<oracle>/surrogate_models/varied_mutrate_nonlin_pairwise/
        ...
        mRNA_RBP/outputs_<oracle>/surrogate_coefs/
            coefs_varied_mutrate_nonlinear_additive_p_pairwise_<oracle>.npz
"""
...
def main():
    ...
    coef_path = args.coef_path or os.path.join(
        ...
        f"coefs_varied_mutrate_nonlinear_additive_p_pairwise_{oracle_name}.npz")
    args.src_path, args.model_dir, args.coef_path = src_path, model_dir, coef_path
