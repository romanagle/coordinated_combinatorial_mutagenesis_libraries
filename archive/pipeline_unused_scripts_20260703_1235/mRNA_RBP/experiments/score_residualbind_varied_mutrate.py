"""mRNA_RBP/score_residualbind_varied_mutrate.py

Scores the EXISTING VTS1-GCUGG varied-mutation-rate library (generated by
generate_varied_mutrate_library.py -- nuc_ids/mut_counts only depend on
the WT sequence/mutation count, never on the oracle) with a real ResidualBind
model instead of the synthetic ground truth. Supports two oracles (see
--oracle): the MSI1 ensemble (default, a linear-motif RRM-domain protein) or
the real VTS1 model (RNCMPT00111 -- structure-sensitive, see oracles.py).
...
Run with the environment matching the chosen oracle:
    ...
    --oracle vts1_residualbind: residbind conda env (TF2.18/Keras3 --
        squid's TF is too old and rbp/.venv's TF segfaults on this machine)
        /home/nagle/miniconda3/envs/residbind/bin/python \
            mRNA_RBP/score_residualbind_varied_mutrate.py --oracle vts1_residualbind

Reads:  mRNA_RBP/outputs/instance_00/varied_mutrate/pool.npz
Writes: mRNA_RBP/outputs_<oracle>/instance_00/varied_mutrate/pool.npz
    nuc_ids            (N, L) uint8   -- passthrough
    mut_counts         (N,)   uint8   -- passthrough
...
def main():
    ...
    parser.add_argument("--overwrite", action="store_true",
                        help="Replace an existing scored pool.npz")
    ...
    if not os.path.exists(args.src_path):
        raise FileNotFoundError(
            f"{args.src_path} not found -- run generate_varied_mutrate_library.py first"
        )
    ...
    nuc_ids, mut_counts = d["nuc_ids"], d["mut_counts"]
    print(f"[score] varied_mutrate pool  n={len(nuc_ids):,}", flush=True)
    scores = _score_nuc_ids(oracle, gt_key, nuc_ids)

    os.makedirs(os.path.dirname(dst_path), exist_ok=True)
    np.savez_compressed(dst_path, nuc_ids=nuc_ids, mut_counts=mut_counts,
