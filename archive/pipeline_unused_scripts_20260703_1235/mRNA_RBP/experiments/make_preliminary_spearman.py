"""mRNA_RBP/make_preliminary_spearman.py

Reads lib_size_spearman_results.json and produces the preliminary
Spearman ρ vs training library size figure.

One panel per surrogate config (or a single panel with --config).
Within each panel:
  - Color  = mutation rate (5% / 10% / 25%)
  - Style  = evaluation split (rand / type1_200,2k,20k / activity-balanced / pairwise)
  - Black dashed horizontal = SSM saturated mutagenesis baseline
...
def main():
    ...
    parser.add_argument(
        "--out",
        default=os.path.join(_HERE, "outputs", "notebook_plots", "preliminary_spearman.png"),
    )
    ...
    parser.add_argument(
        "--mut_rates", nargs="+", type=int, default=MUT_RATES,
        help="Mutation rates to include (default: 5 10 25)",
    )
