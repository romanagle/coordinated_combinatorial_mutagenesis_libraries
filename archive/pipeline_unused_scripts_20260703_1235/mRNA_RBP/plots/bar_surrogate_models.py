...
Eval libraries (bar pair per group):
  activity-balanced library (blue) | pairwise eval library (orange)
...
from mRNA_RBP.generate_libraries import (
    SEQ, STEM_PAIRS, MOTIF_POSITIONS, OUT_BASE, STEM_SIGMA, activity_balanced_path,
)
...
def main(save_svg: bool = False):
    ...
    print(f"SSM      activity-balanced={ssm_t2:.3f}  pairwise={ssm_pw:.3f}")
    ...
    for label, cfg in SURROGATE_CONFIGS.items():
        ...
        print(f"  activity-balanced={rho_t2:.3f}  pairwise={rho_pw:.3f}")
    ...
    bars_t2 = ax.bar(x - BAR_WIDTH / 2, means_t2, BAR_WIDTH,
                     color=COLOR_TYPE2,    label="Activity-balanced library")
    ...
    plt.close(fig)
    print(f"\nSaved → {out}" + (" / bar_surrogate_models.svg" if save_svg else ""))
