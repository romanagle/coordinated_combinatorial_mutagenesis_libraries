...
Generate copies of the coefficient plots with a full 4L×4L pairwise panel:
  outputs/notebook_plots/coefficients_gt_4Lx4L.png (add --svg for .svg too)
  outputs/notebook_plots/coefficients_surrogate_4Lx4L.png (add --svg for .svg too)
...
def main():
    ...
    out_gt = os.path.join(OUT_DIR, "coefficients_gt_4Lx4L.png")
    fig_gt.savefig(out_gt, dpi=150, bbox_inches="tight")
    print(f"Saved → {out_gt}")
    if args.svg:
        out_gt_svg = os.path.join(OUT_DIR, "coefficients_gt_4Lx4L.svg")
        fig_gt.savefig(out_gt_svg, bbox_inches="tight")
        print(f"Saved → {out_gt_svg}")
    plt.close(fig_gt)
    ...
    out_sur = os.path.join(OUT_DIR, "coefficients_surrogate_4Lx4L.png")
    fig_sur.savefig(out_sur, dpi=150, bbox_inches="tight")
    print(f"Saved → {out_sur}")
    if args.svg:
        out_sur_svg = os.path.join(OUT_DIR, "coefficients_surrogate_4Lx4L.svg")
        fig_sur.savefig(out_sur_svg, bbox_inches="tight")
        print(f"Saved → {out_sur_svg}")
    plt.close(fig_sur)
