...
Generate outputs/notebook_plots/pairwise_heatmaps_annotated.png:
  3-row pairwise evaluation heatmap (GT vs surrogate vs SSM baseline)
  for the first 3 stem pairs, with a colorbar scale and cell-value annotations.
...
def main():
    ...
    parser.add_argument("--out", default=os.path.join(
        _HERE, "outputs", "notebook_plots", "pairwise_heatmaps_annotated.png"))
    ...
    if np.isfinite(rho_ssm):
        ssm_label = f"SSM baseline\nρ={rho_ssm:.2f}"
    else:
        ssm_label = "SSM baseline\nρ=undefined"
