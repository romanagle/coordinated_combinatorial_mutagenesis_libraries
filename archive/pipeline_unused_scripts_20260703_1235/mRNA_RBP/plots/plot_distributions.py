...
def run_plots(gt_key: str = GT_KEY, save_svg: bool = False):
    ...
    for pct in MUT_RATES_PCT:
        for n_lib in LIB_SIZES:
            ...
            msg = out_path
            if save_svg:
                svg_path = os.path.join(plots_dir, f"mut{pct:02d}_lib{n_lib}.svg")
                plt.savefig(svg_path)
                msg = f"{out_path} / .svg"
            plt.close(fig)
            print(f"  mut{pct:02d}%  lib{n_lib:>6,}  →  {msg}")
