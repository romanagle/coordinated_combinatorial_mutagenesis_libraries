...
from plotting import (
    ...
    plot_spearman_violin_4subplots, plot_spearman_violin_from_grid,
)
...
def random_mut_library_round0(rho_storage, coef_store, run_idx, epmapmin, epmapmax):
    ...
    x_mut, y_mut = mave.generate(no_padding_seq, padding_amt=padding_amt, num_sim=20000)
    ...
    plot_spearman_violin_from_grid(
        grid_results,
        gt_keys=gt_keys_local,
        cfg_names=list(SURROGATE_CONFIGS.keys()),
        out_path=(f"/home/nagle/final_version/outputs/{subdir}/{activity}"
                  f"/spearman_violin_run{run_idx}.png"),
...
# Save rho_storage as JSON for offline analysis / re-plotting
import json as _json
_rho_json_path = (f"/home/nagle/final_version/outputs/{subdir}/{activity}"
                  f"/rho_storage_step0.json")
def _rho_to_serialisable(obj):
    if isinstance(obj, dict):
        return {k: _rho_to_serialisable(v) for k, v in obj.items()}
    if isinstance(obj, (list, tuple)):
        return [_rho_to_serialisable(x) for x in obj]
    ...
    if isinstance(obj, np.integer):
        return int(obj)
    return obj
with open(_rho_json_path, "w") as _f:
    _json.dump(_rho_to_serialisable(rho_storage), _f, indent=2)
print(f"[saved] rho_storage → {_rho_json_path}")

# Violin plots: Spearman ρ distributions per GT type
plot_spearman_violin_4subplots(
    rho_storage,
    out_path=(f"/home/nagle/final_version/outputs/{subdir}/{activity}"
              f"/spearman_violin_step0.png"),
    experiment=experiment,
    step="0",
    gt_keys=gt_keys,
    cfg_names=cfg_names,
