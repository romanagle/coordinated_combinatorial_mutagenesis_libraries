...
parser.add_argument("--lefttail", type=str)
parser.add_argument("--righttail", type=str)
parser.add_argument("--motif", type=str)
parser.add_argument("--activity", type=str, help='TESTING low vs high activity')
parser.add_argument("--mut_rate", type=int, default=2, help='Integer value for how many positions you want to mutate in a given sequence')
parser.add_argument("--lib_size", type=int, default=20000, help='How many mutagenized sequences you want in your library')


args = parser.parse_args()

left_tail = args.lefttail
right_tail = args.righttail
motif = args.motif
num_muts = args.mut_rate
lib_size = args.lib_size
actvitiy = args.activity
...
def ep_map(iteration, x_lib, y, reg_strength, noise_order):
...
oh_seq_left = rna_to_one_hot(left_tail)
oh_seq_right = rna_to_one_hot(right_tail)
oh_seq_motif = rna_to_one_hot(motif)
no_padding_seq_left, _ = remove_padding(oh_seq_left) #needs to be in one-hot encoded form
no_padding_seq_right, _ = remove_padding(oh_seq_right) #needs to be in one-hot encoded form
no_padding_seq_motif, _ = remove_padding(oh_seq_motif) #needs to be in one-hot encoded form

padding_amt_l = 41 - oh_seq_left.shape[0] #residualbind requires that the sequence is 41 nucleotides long
padding_amt_r = 41 - oh_seq_right.shape[0] #residualbind requires that the sequence is 41 nucleotides long
padding_amt_m = 41 - oh_seq_motif.shape[0] #residualbind requires that the sequence is 41 nucleotides long


seq_length_l = no_padding_seq_left.shape[0]
seq_length_r = no_padding_seq_right.shape[0]
seq_length_m = no_padding_seq_motif.shape[0]
mut_window_l = [0, seq_length_l]
mut_window_r = [0, seq_length_r]
...
mave_m = squid.mave.InSilicoMAVE(mut_generator, pred_generator, seq_length_l, mut_window=mut_window_l)
#mave_r = squid.mave.InSilicoMAVE(mut_generator, pred_generator, seq_length_l, mut_window=mut_window_r)
#x_mut_l, y_mut = mave_l.generate(no_padding_seq_left, padding_amt=padding_amt_l, num_sim=lib_size)
x_mut_m, y_mut = mave_m.generate(no_padding_seq_motif, padding_amt=padding_amt_m, num_sim=lib_size)
#x_mut_r, y_mut = mave_r.generate(no_padding_seq_right, padding_amt=padding_amt_r, num_sim=lib_size)

#left is mutated, right and motif stays the same
x_mut = np.concatenate([np.broadcast_to(no_padding_seq_left, (lib_size, seq_length_l, 4)), x_mut_m, np.broadcast_to(no_padding_seq_right, (lib_size, seq_length_r, 4))], axis=1)

seqs_with_padding, prediction = predictor(x_mut, "original", plot=False)
with open("/home/nagle/final_version/output/sequences_mutate_tails.txt", "w") as f:
...
input_seq = left_tail + motif + right_tail
...
# save the figure
plt.savefig("/home/nagle/final_version/output/mutate_middle.png")